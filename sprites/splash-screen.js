(function() {
	...
	SplashScreen.prototype = Object.create(SpaceInvadersNamespace.Sprite.prototype);
	...
	function SplashScreen(config) {
		...
		that.logo = imageManager.getImage(LOGO_FILENAME);

		that.continueTextVisible = true;
		that.lastTime = Date.now();

		that.__type = "SplashScreen";
	}

	SplashScreen.prototype.update = function(lastFrameEllapsedTime, keyboard) {
		let that = this,
			now = Date.now(),
